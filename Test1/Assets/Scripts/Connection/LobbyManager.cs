...
using ExitGames.Client.Photon; // Photon¿¡¼­ »ç¿ëÇÏ´Â 
...
public class LobbyManager : MonoBehaviourPunCallbacks
{
    ...
    // ¹æ »ı¼º °ü·Ã UI
    public GameObject RoomSet_A_Panel; //È¥ÀÚÇÏ±â ¹æ ÆĞ³Î(È°¼ºÈ­ ¿©ºÎ¿¡ µû¸¥ Ã³¸®)

    [SerializeField] TMP_InputField input_RoomName; //¹æ ÀÌ¸§

    [SerializeField] Button[] btn_MaxPlayers, btn_Difficulty, btn_TimeLimit; // ÃÖ´ëÀÎ¿ø, ³­ÀÌµµ, Á¦ÇÑ½Ã°£ ¹öÆ°
    [SerializeField] Button[] btn_Difficulty_A, btn_TimeLimit_A; // ÃÖ´ëÀÎ¿ø, ³­ÀÌµµ, Á¦ÇÑ½Ã°£ ¹öÆ°(È¥ÀÚ ÇÏ±â ÆĞ³Î)

    [SerializeField] Image[] PlayerSel, DiffSel, TimeSel; //¼±ÅÃµÈ »óÅÂ¸¦ ³ªÅ¸³»´Â ÀÌ¹ÌÁö ¹è¿­
    [SerializeField] Image[] DiffSel_A, TimeSel_A; //¼±ÅÃµÈ »óÅÂ¸¦ ³ªÅ¸³»´Â ÀÌ¹ÌÁö ¹è¿­(È¥ÀÚ ÇÏ±â ÆĞ³Î)

    // ¹æ »ı¼º ½Ã ÀÌ¸§ ±ÔÄ¢ °æ°í¸Ş½ÃÁö
    [SerializeField] TMP_Text warningText;

    // ¹æ »ı¼º ¹öÆ°°ú ¹æ Âü¿© ¹öÆ°, ¹æ ¸ñ·ÏÀ» Ç¥½ÃÇÒ ½ºÅ©·Ñºä
    public Button btn_1, btn_2; // ¹æ ¸¸µé±â, Âü¿© ¹öÆ°(ÃÊ·Ï»ö)
    ...
    public string selectedDifficulty = "ÃÊ±Ş"; // ³­ÀÌµµ(ÃÊ±Ş, Áß±Ş, °í±Ş)
    ...
    private void Awake()
    {
        ...
        if (PhotonNetwork.InLobby) //ÇöÀç ·Îºñ¿¡ ÀÖ´Ù¸é
        {
            Debug.Log("·Îºñ¿¡ ÀÖÀ½");
            Main.instance.GetProfileImageIndex(); // PlayFab¿¡¼­ ÀúÀåµÈ ÀÌ¹ÌÁö ÀÎµ¦½º¸¦ ºÒ·¯¿Í ÀÌ¹ÌÁö ¾÷µ¥ÀÌÆ®
            Main.instance.GetUserDisplayName(); //À¯Àú ³×ÀÓ ºÒ·¯¿Í¼­ ÅØ½ºÆ®·Î Ç¥½Ã
            Main.instance.GetUserBirth(); //À¯Àú Ãâ»ı¿¬µµ ºÒ·¯¿À±â¸¸ ÇÏ±â
            Main.instance.profilePanel.SetActive(false); //ÇÁ·ÎÇÊ ÆĞ³Î ºñÈ°¼ºÈ­
        }
        else
        {
            Debug.Log("·Îºñ¿¡ ¾øÀ½");

            if (CheckLobbyManager.instance.Beforescene == "Default") //ÀÌ¶§ Áö±İÀÌ Ã¹ ·Î±×ÀÎÀ¸·Î µé¾î¿Â °æ¿ì¶ó¸é
            {
                Debug.Log("ÀÚµ¿·Î±×ÀÎ ¼­¹ö ¿¬°á ÁøÇà Áß...");
            }
            else //ÇÃ·¹ÀÌ·ëÀÌ³ª Ã¤ÆÃ¹æ¿¡ ÀÖ´Ù°¡ ¿Â°Å¶ó¸é
            {
                //Debug.Log("¸¶½ºÅÍ ¼­¹ö¿¡ ¿¬°áµÆÀ» ¶§ ·Îºñ ÀÚµ¿ Á¢¼ÓÀ» À§ÇØ ´ë±â Áß...");

                if (PhotonNetwork.NetworkClientState == ClientState.ConnectedToMasterServer)
                {
                    Debug.Log("ÇöÀç ¸¶½ºÅÍ ¼­¹ö¿¡ ¿¬°áµÇ¾î ÀÖÀ½");

                    if (PhotonNetwork.InLobby)
                    {
                        Debug.Log("ÀÌ¹Ì ·Îºñ¿¡ ÀÖÀ½");
                    }
                    else
                    {
                        Debug.Log("·Îºñ ¿¬°á ½Ãµµ");
                ...
                {
                    Debug.Log("¸¶½ºÅÍ ¼­¹ö¿¡ ¿¬°áµÇ¾î ÀÖÁö ¾ÊÀ½. ¿¬°á ½Ãµµ Áß...");
    ...
    private void Start()
    {
        ResetRoomSetPanel(); //¹æ »ı¼º ÆĞ³Î ÃÊ±âÈ­
        ResetRoomSetPanel_Alone(); //ÆĞ³ÎA ÃÊ±âÈ­

        //if (CheckLobbyManager.instance.Beforescene == "PlayRoom") // ÇÃ·¹ÀÌ¸¦ ÇÑ¹ø ÇÏ°í¿Â°Å¶ó¸é
        //{
    ...
    {
        //ÀÚµ¿·Î±×ÀÎ Ã¹ ÁøÀÔÀÌ ¾Æ´Ò °æ¿ì¿¡¸¸ ¼öÇà
        if (CheckLobbyManager.instance.Beforescene != "Default")
        {
            Debug.Log("¸¶½ºÅÍ ¼­¹ö ¿¬°áµÊ ¡æ ·Îºñ Á¢¼Ó ½Ãµµ");
            PhotonNetwork.JoinLobby();
        }
    }
    ...
    // ¹æ ¸¸µé ¶§ ¼±ÅÃ ¿É¼Ç ¹öÆ°°ú ¹æÀÌ¸§ ±ÔÄ¢¿¡ °üÇÑ ÃÊ±âÈ­(¹æ ¼Ó¼º x¹öÆ° ´©¸¦¶§µµ ½ÇÇà-ÃÊ±âÈ­)
    ...
    public void ResetRoomSetPanel_Alone()
    {
        ...
        UnityEngine.Debug.Log("¸ŞÀÎÈ­¸é ÃÊ±âÈ­ ¿Ï·á_A");
    }
    ...
    private void MaxPlayerSet(Button[] buttons)
    {
        ...
        {
            int index = i;
            buttons[i].onClick.RemoveAllListeners(); // ±âÁ¸ ÀÌº¥Æ® Á¦°Å
    ...
    private void SetDefaultSelection(int defaultIndex)
    {
        ...
        for (int i = 0; i < 3; i++) //¼¼¹ø ¹İº¹(°¢ ¹öÆ° ¹è¿­ ±æÀÌ)
        {
            if (i == defaultIndex) //ÇöÀç ¼±ÅÃÇÑ ÀÎµ¦½º(0)¿Í i°ªÀÌ °°À»¶§
            {
    ...
    private void UpdateButtonColors(Button[] buttons, int selectedIndex)
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            {
                if (buttons == btn_MaxPlayers) //ÃÖ´ëÀÎ¿ø ¹öÆ° ¼±ÅÃ
                {
                    PlayerSel[i].gameObject.SetActive(true);
                }
                else if (buttons == btn_TimeLimit || buttons == btn_TimeLimit_A) //Á¦ÇÑ½Ã°£ ¹öÆ° ¼±ÅÃ
                {
                ...
                else if (buttons == btn_Difficulty || buttons == btn_Difficulty_A) //³­ÀÌµµ ¹öÆ° ¼±ÅÃ
                {
                    if (RoomSet_A_Panel.gameObject.activeSelf)
                    {
                        DiffSel_A[i].gameObject.SetActive(true);
    ...
    void UpdateRoomListItem(List<RoomInfo> roomList)
    {
        ...
        UnityEngine.Debug.Log($"[DEBUG] ÇöÀç ¹æ °³¼ö: {dicRoomInfo.Count}");
    ...
    void CreateRoomListItem()
    {
        ...
        //UnityEngine.Debug.Log($"»ı¼ºµÈ ¹æ UI °³¼ö: {count}");
    }
    ...
    public void OnClickCreateRoom()
    {
        ...
        options.CustomRoomProperties = new ExitGames.Client.Photon.Hashtable()
        {
           //³­ÀÌµµ, Á¦ÇÑ½Ã°£ °¢°¢¿¡ ´ëÇÑ ÀÎµ¦½º¿Í ½ÇÁ¦ ÅØ½ºÆ®·Î ¹İ¿µµÉ °ªµé
            ...
            {"DifficultyContents", Changelevel.cardFrontBlack.ToArray() }, // ³­ÀÌµµ Ä«µå ³»¿ë(°¡,°¼,°Å..), Àü´ŞÀ» À§ÇØ ¹è¿­·Î ÇüÅÂº¯°æ
            {"TimeLimitIndex", selectedTimeLimitIndex},  // Á¦ÇÑ½Ã°£ index
            {"timeLimit", selectedTimeLimit}  // Á¦ÇÑ½Ã°£ int°ª(15,30,45)
        };

        if (!string.IsNullOrWhiteSpace(input_RoomName.text)) //2ÀÎ ÀÌ»ó ¹æÀ» ¸¸µç´Ù¸é(¹æÀÌ¸§ Á¸Àç) ·Îºñ¿¡ º¸ÀÌ°Ô
        {
            //¹æ »ı¼º
    ...
    {
        if (!string.IsNullOrEmpty(selectedRoomName)) //¹æÀÌ¸§ÀÌ ¹¹¶óµµ ÀÖÀ¸¸é(´ÙÀÎ¿ø ¹æ)
        {
    ...
    public override void OnJoinedRoom() // ¹æ¿¡ ÀÔÀåÇßÀ» ¶§ ÀÚµ¿ È£Ãâ
    {

        UserInfoManager.instance.MyActNum = PhotonNetwork.LocalPlayer.ActorNumber; //¾×ÅÍ³Ñ¹ö

        Hashtable update = new Hashtable
        {
            { "PlayerJoined", UserInfoManager.instance.MyActNum }, //¾×ÅÍ³Ñ¹ö
            { "PlayerName", UserInfoManager.instance.MyName }, //³» ÀÌ¸§
            { "PlayerProfile", UserInfoManager.instance.MyImageIndex } //»çÁø ÀÎµ¦½º
        };

        PhotonNetwork.CurrentRoom.SetCustomProperties(update);
    }

    public override void OnRoomPropertiesUpdate(Hashtable propertiesThatChanged) //¹æ¿¡ Ã³À½ µé¾î¿ÔÀ» ¶§¸¸ ¼öÇàµÇ´Â ÇÔ¼ö
    {
        if (propertiesThatChanged.ContainsKey("PlayerJoined"))
        {
            ...
            // º»ÀÎÀº Á¦¿Ü (ÀÌ¹Ì ¹æ ÀÔÀå ½ÃÁ¡¿¡ OnJoinedRoom¿¡¼­ Ã³¸®ÇßÀ¸¹Ç·Î)
            //if (joinedActorNumber != PhotonNetwork.LocalPlayer.ActorNumber)
            //{
            //      TODO: »õ À¯Àú ÀÔÀå °¨Áö ÈÄ Çàµ¿ ½ÇÇà
            //      Debug.Log($"ÇÃ·¹ÀÌ¾î ³Ñ¹ö: {joinedActorNumber} ÀÔÀå!");
            //      Debug.Log($"ÇÃ·¹ÀÌ¾î ÀÌ¸§ {joinedName}");
            //      Debug.Log($"ÇÃ·¹ÀÌ¾î »çÁø {joinedImageIndex}");
            //}

            //TODO: À¯Àú ÀÔÀå °¨Áö ÈÄ Çàµ¿ ½ÇÇà
            //Debug.Log($"ÇÃ·¹ÀÌ¾î ³Ñ¹ö: {joinedActorNumber} ÀÔÀå!");
            //Debug.Log($"ÇÃ·¹ÀÌ¾î ÀÌ¸§ {joinedName}");
            //Debug.Log($"ÇÃ·¹ÀÌ¾î »çÁø {joinedImageIndex}");

            if (input_RoomName.text == "Default") //ÇöÀç È¥ÀÚÇÏ±â·Î ¹æ¿¡ µé¾î¿Â°Å¶ó¸é
            {
                //¹Ù·Î ÇÃ·¹ÀÌ·ëÀ¸·Î ÀÌµ¿
