...
{
    [SerializeField] public TMP_InputField inputText, birthInput; //´Ğ³×ÀÓ ÀÔ·Â(³ªÁß¿¡ ¹Ù²Ü ¼ö ÀÖ´Â Displayname), Ãâ»ı¿¬µµ ÀÎÇ²
    [SerializeField] public Button Next2, confirmButton; //Á¦Ãâ(ÀúÀå) ¹öÆ°
    [SerializeField] public TMP_Text warningText, birthWarningText; // °æ°í ¸Ş½ÃÁö¸¦ Ãâ·ÂÇÒ UI ÅØ½ºÆ®
    [SerializeField] public TMP_Text saveText; // ÀúÀå¿Ï·á ¸Ş½ÃÁö¸¦ Ãâ·ÂÇÒ UI ÅØ½ºÆ®(profile panel¿¡¸¸ Á¸Àç)
    ...
    void Start()
    {
        ...
        confirmButton.onClick.AddListener(SaveBirthInfo); // Ãâ»ı¿¬µµ ÀúÀå
        confirmButton.onClick.AddListener(SaveDisplayName); //ÀÌ¸§ ÀúÀå ¹× ¸ŞÀÎ Á¢¼Ó ¿©ºÎ ¼³Á¤
        confirmButton.onClick.AddListener(() => { inputText.interactable=false; }); // ÀÎÇ²ÇÊµå ¼±ÅÃ ºñÈ°¼ºÈ­
    }
    ...
    private void LoadDefaultImageIndex() // ÀúÀåµÈ ÀÌ¹ÌÁö ÀÎµ¦½º¸¦ ºÒ·¯¿À±â(Á¸ÀçÇÏ¸é ÇØ´ç °ªÀ», Á¸ÀçÇÏÁö ¾ÊÀ¸¸é ±âº»°ª 0)
    {
        ...
        centralImage.sprite = UserInfoManager.instance.profileImages[currentIndex];  // ÀÌ¹ÌÁö ¾÷µ¥ÀÌÆ®
    }
    ...
    public void SaveDisplayName() //DisplayNameÀ» playfab°ú playerprefs¿¡ ÀúÀå
    {
        ...
        Debug.Log($"[playerprefs] Displayname: {UserInfoManager.instance.MyName}À» ÀúÀåÇß½À´Ï´Ù");
    ...
    {
        UserInfoManager.instance.MyImageIndex = currentIndex; // intÇü

        string ImageIndex = UserInfoManager.instance.MyImageIndex.ToString(); // ¹®ÀÚ¿­·Î º¯È¯
        ...
        PlayFabClientAPI.UpdateUserData(request,
            ...
            error =>
            {
                Debug.LogError($"[Playfab] À¯Àú ÀÌ¹ÌÁö ÀúÀå ½ÇÆĞ: {error.GenerateErrorReport()}");
            });

        // º¯°æµÈ ÀÌ¹ÌÁö ÀÎµ¦½º¸¦ playerprefs¿¡ ÀúÀå(±âÁ¸ À¯ÀúÀÇ °æ¿ì µ¤¾î¾²±â, ½Å±Ô À¯Àú´Â »õ·Î Ãß°¡ÇÏ´Â »óÈ²)
        UpdateImageIndex(UserInfoManager.instance.MyImageIndex);
        Debug.Log($"[playerprefs] Imageindex: {UserInfoManager.instance.MyImageIndex}À» ÀúÀåÇß½À´Ï´Ù");
    ...
    private void SaveBirthInfo() //ÀÔ·ÂÇÑ Ãâ»ı¿¬µµ¸¦ ÀúÀå(°ªÀ» ÀÔ·ÂÇßÀ» °æ¿ì¿¡¸¸)
    {
        if (birthInput.text.Length == 4)
        {
            string BirthYear = UserInfoManager.instance.MyBirthYear.ToString(); // ¹®ÀÚ¿­·Î º¯È¯
            ...
            PlayFabClientAPI.UpdateUserData(request,
                result =>
                {
                    Debug.Log($"[Playfab] Ãâ»ı¿¬µµ µ¥ÀÌÅÍ ÀúÀå ¼º°ø: {BirthYear}");
                },
                error =>
                {
                    Debug.LogError($"[Playfab] Ãâ»ı¿¬µµ ÀúÀå ½ÇÆĞ: {error.GenerateErrorReport()}");
                });
    ...
    {
        PlayerPrefs.SetString(UserInfoManager.DISPLAYNAME_KEY, name); // »õ·Î¿î °ª ÀúÀå
        PlayerPrefs.Save(); // ÀúÀå À¯Áö
    }

    void UpdateImageIndex(int newIndex) //»õ·Î¿î ÀÎµ¦½º ÀúÀå
    {
        PlayerPrefs.SetInt(UserInfoManager.IMAGEINDEX_KEY, newIndex); // »õ·Î¿î °ª ÀúÀå
        PlayerPrefs.Save(); // ÀúÀå À¯Áö
    }

    void UpdateBirthYear(int newIndex) //»õ·Î¿î ÀÎµ¦½º ÀúÀå
    {
        PlayerPrefs.SetInt(UserInfoManager.BIRTHYEAR_KEY, newIndex); // »õ·Î¿î °ª ÀúÀå
        PlayerPrefs.Save(); // ÀúÀå À¯Áö
    }
    ...
    public void OnLeftButtonClicked() // ¿ŞÂÊ ¹öÆ° Å¬¸¯ ½Ã È£Ãâ
    {
        ...
        centralImage.sprite = UserInfoManager.instance.profileImages[currentIndex];  // ÀÎµ¦½º¿¡ ÇØ´çÇÏ´Â ÀÌ¹ÌÁö·Î ¾÷µ¥ÀÌÆ®
        Debug.Log($"ÇöÀç ÀÎµ¦½º: {currentIndex}");
        if (UserInfoManager.instance.MyImageIndex == currentIndex) //¸¸¾à ÇöÀç ÀÎµ¦½º ÀÌ¹ÌÁö¿Í ±âÁ¸ ÀÌ¹ÌÁö ÀÎµ¦½º°¡ °°´Ù¸é
        {
            confirmButton.interactable = false; //ÀúÀå¹öÆ° ºñÈ°¼ºÈ­
        }
        else
        {
    ...
    public void ValidateNickname(string input) //ÀÌ¸§ ¼³Á¤ ±ÔÄ¢(displayname)
    {
        ...
        if (!Regex.IsMatch(inputname, validPattern))
        {
            ...
            if (profilePanel.activeSelf) //ÇÁ·ÎÇÊ º¯°æÀÏ¶§
            {
                confirmButton.interactable = false;
            }
            else //À¯ÀúÃÊ±â ¼³Á¤ÀÏ¶§
        ...
        else if (UserInfoManager.instance.MyName == inputname && (inputText.isActiveAndEnabled)) //ÀÔ·Â¶õÀÌ ±âÁ¸ ´Ğ³×ÀÓ°ú °°À¸¸é¼­ È°¼ºÈ­µÇ¾îÀÖ´Â °æ¿ì
        {
    ...
    public void ValidateBirth(string input) //Ãâ»ı¿¬µµ Çü½Ä¿¡ ¸Â°Ô ÀÔ·ÂÇß´ÂÁö °Ë»ç
    {
        // ³× ÀÚ¸® ¼ıÀÚÀÎÁö °Ë»ç
        if (int.TryParse(input, out int year) && input.Length == 4)
        {
            // ÀûÀıÇÑ ¹üÀ§¿¡ ÀÖ´ÂÁö °Ë»ç
            if (year >= 1945 && year <= 2020) 
            {
                birthWarningText.text = "";
                UserInfoManager.instance.MyBirthYear = year; //À¯ÀúÀÎÆ÷¿¡ ¹Ù·Î ÀúÀå
                confirmButton.interactable = true; // ¿Ã¹Ù¸¥ Çü½ÄÀÌ¸é ¹öÆ° È°¼ºÈ­
            }
            else
            {
                birthWarningText.text = "Á¤È®ÇÑ Á¤º¸¸¦ ÀÔ·ÂÇØÁÖ¼¼¿ä";
                confirmButton.interactable = false; // Àß¸øµÈ Çü½ÄÀÌ¸é ¹öÆ° ºñÈ°¼ºÈ­
            }
        }
        else
        {
            birthWarningText.text = "4ÀÚ¸® ¼ıÀÚ¸¸ ÀÔ·ÂÇØÁÖ¼¼¿ä";
            confirmButton.interactable = false; // Àß¸øµÈ Çü½ÄÀÌ¸é ¹öÆ° ºñÈ°¼ºÈ­
        }
    }
