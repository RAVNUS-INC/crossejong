...
public class UserSetManager : MonoBehaviourPunCallbacks
{
    ...
    {
        PlayerPrefs.SetString(UserInfoManager.instance.DISPLAYNAME_KEY, name); // »õ·Î¿î °ª ÀúÀå
        PlayerPrefs.Save(); // ÀúÀå À¯Áö
    }

    void UpdateImageIndex(int newIndex) //»õ·Î¿î ÀÎµ¦½º ÀúÀå
    {
        PlayerPrefs.SetInt(UserInfoManager.instance.IMAGEINDEX_KEY, newIndex); // »õ·Î¿î °ª ÀúÀå
        PlayerPrefs.Save(); // ÀúÀå À¯Áö
    }
    ...
    {
        currentIndex--;
        if (currentIndex < 0) currentIndex = UserInfoManager.instance.profileImages.Length - 1;  // ¼øÈ¯ (¸Ç Ã³À½À¸·Î µ¹¾Æ°¨)
